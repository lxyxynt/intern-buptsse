...
var announcement = 'no announcement';
...
app.post("/announcement", function (req, res, next) {
    /* maybe should be comment in testing
...
app.get("/announcement", function (req, res, next) {
    ...
    if (req.session.user.role != 'professor')
        res.send(403, {err:'access denied'}); 
    //*/
    return res.send(200, {text: announcement});
});
...
app.get("/u/:uid/profile", function (req, res, next) {
    ...
    User.findOne({username: uid}, function (err, user) {
        ...
        ret.resume = user.resume;
        ret.homepage = user.homepage;
...
app.post("/u/:uid/profile", function (req, res, next) {
    ...
    User.findOne({username: uid}, function (err, user) {
        ...
        var ret = {};
        user.realname = ret.realname = args.realname || user.realname;
        user.email = ret.email = args.email || user.email;
        user.phone = ret.phone = args.phone || user.phone;
        user.resume = ret.resume = args.resume || user.resume;
        user.homepage = ret.homepage = args.homepage || user.homepage;
...
app.post("/passwd", function (req, res, next) {
    if (!req.session.user) return res.send(403, {err: 'access_denied'});

    User.findOne({username: req.session.user.username}, function (err, user) {
        if(err) return next(err);
        if(!user) return res.send(403, {err: 'access_denied'});

        var args = req.body;
        if(!passwordHash.verify(args.old, user.password))
            return res.send(403, {err: 'access_denied'});
        
        user.password = passwordHash.generate(args.new);
        user.save(function(err) {
            if (err) {
                res.send(400, {err: 'save failed'});        
            }
